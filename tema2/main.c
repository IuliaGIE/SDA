...
/*
void printTreeByLevels(QuadTreeNode* root, int level) {
    if(root == NULL){
        return;
    }
    printf("Level %d: %d %d %d\n", level, root->x, root->y, root->size);
    level++;
    for(int i = 0; i < 4; i++){
        printTreeByLevels(root->children[i], level);
    }
}
*/
    ...
    if (strcmp(argv[1], "-c1") == 0)
    {
        ...
        for (i = 0; i < height; i++)
        {
            for (j = 0; j < width; j++)
            {
                ...
                //fread(&image[i][j].red, sizeof(unsigned char), 1, in);
                //fread(&image[i][j].green, sizeof(unsigned char), 1, in);
                //fread(&image[i][j].blue, sizeof(unsigned char), 1, in);
            }
        }
        //printf("%u\n", image[i-1][j-1]. red);
        // ... prelucrare ...
        QuadTreeNode *root = buildQuadTree(image, 0, 0, width, factor);
        //printTreeByLevels(root, 0);
        int nivel, frunze, dimensiune;
        //nivel = num_levels(root);
        //dimensiune = getMaxLeafSize(root);
